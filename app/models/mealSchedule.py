from . import db
from app.models.recipe import Recipe  # Importar el modelo Recipe para la relación
...
class MealSchedule(db.Model):
    ...
    recipe_id = db.Column(db.Integer, db.ForeignKey('recipe.id'), nullable=False)  # Relacion con Recipe

    # Relación con Recipe
    recipe = db.relationship('Recipe', backref=db.backref('meal_schedules', lazy=True))

    def __repr__(self):
        return f"<MealSchedule(unique_id={self.unique_id}, week_day='{self.week_day}', hour={self.hour}, training_plan='{self.training_plan}', recipe_id={self.recipe_id})>"
